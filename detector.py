...
class CarDetector(object):
    def __init__(self):
        ...
        detect_model_name = '' # set model name
        ...
        # load frozen tensorflow detection model 
        # and initialize the tensorflow graph
    ...
    def get_localization(self, image, visual=False):
        ...
        with self.detection_graph.as_default():
              ...
              # The ID for car is 3
              idx_vec = [i for i, v in enumerate(cls) if ((v==3) and (scores[i]>0.57))]
              print(max(scores))
              ...
              if len(idx_vec) ==0:
                  ...
                  if len(idx_vec) > 1:
                      idx_vec = [idx_vec[scores.argmax(axis=0)]]
                  print(idx_vec)
                  for idx in idx_vec:
                      dim = image.shape[0:2]
                      ...
                      if ((ratio > 0.5) and (box_h>20) and (box_w>20)):
...
if __name__ == '__main__':
        det =CarDetector()
        os.chdir(cwd)
        TEST_IMAGE_PATHS= glob(os.path.join(''))
